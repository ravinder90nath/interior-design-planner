...
const CanvasBoard = ({ canvasRef }) => {
  ...
  const { items, blueprintImg, selectedId, setSelectedId, moveItem } =
    useLayers();
  ...
  const getItem = useCallback(
    (id) => items.find((i) => i.id === id) || { xPct: 0, yPct: 0 },
    [items],
  );
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      if (e.key === "Escape" && activeTool === TOOLS.POLYGON)
        polyDraw.cancelPolygon();
    };
    ...
    >
        ...
        >
          <Box
            sx={{
              ...
              border: `2px dashed ${tk.accent}30`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
            }}
      ...
      {activeTool === TOOLS.POLYGON && (
        ...
        >
          ...
          >
            CLICK TO ADD POINTS · CLICK FIRST POINT OR DOUBLE-CLICK TO CLOSE ·
            ESC TO CANCEL
          </Typography>
        </Box>
      )}
